...
void DrawTime() {
  ...
  snprintf(bufDate2, sizeof(bufDate2), "%02d-%02d-%04d",
           timeinfo.tm_mday,
           timeinfo.tm_mon + 1,
           timeinfo.tm_year + 1900);
  ...
  drawString(SCREEN_WIDTH / 2, 0, "Today: " + dateStr2, CENTER);
...
void FetchCalendar() {
  ...
  if (httpGetJson(API_RACES.c_str())) {
    ...
    String today = dateStr;  // must be YYYY-MM-DD format to fetch dtaa
...
void DrawDrivers() {
  httpGetJson(API_DRIVER_STAND.c_str());
  {
    ...
    for (int i = 0; i < 10 && i < ds.size(); i++) {
      ...
      u8g2Fonts.setForegroundColor(GxEPD_RED);
      drawString(listx, 13, "Top 10 Drivers", LEFT);
      u8g2Fonts.setForegroundColor(GxEPD_BLACK);
